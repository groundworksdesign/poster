...
export const parseSongXML = async (xmlContent: string): Promise<SongData> => {
  try {
    ...
    let songElement = xmlDoc.querySelector('song') || xmlDoc.querySelector('Song');
    if (!songElement) throw new Error('Invalid song XML format: missing song element');
    ...
    if (legacyVerseElements.length === 0) legacyVerseElements = songElement.querySelectorAll(':scope > verse');
...
export const createSongSlides = (songData: SongData, existingStyle: any) => {
  ...
  songData.verses.forEach(v => v.lines.forEach(l => allLines.push({ verse: v.number, text: l })));
  const slides: any[] = [];
