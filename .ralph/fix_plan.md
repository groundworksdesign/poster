...
| Song staged advance (title → lyric pairs → blank) | Partial | `handleSongStageAdvance`, `stagedSongSlide.ts` — **wraps to stage 0 instead of next slide** |
...
### 1. REQ-001 — Song advance after last lyric → next deck slide
**Gap:** `handleSongStageAdvance` uses `(last + 1) % total`, which wraps to stage 0 (song restart). `navigatePresentationNext` always delegates to `handleSongStageAdvance` when the presented slide is a song, so Next / right-arrow / row Advance all restart the song after the terminal blank stage instead of calling `sendSlideAtIndex(current + 1)`.
**Tests:** No unit test asserts last-lyric → next non-song slide. `stagedSongSlide.test.ts` only covers staging math, not deck navigation.

### 2. REQ-002 — Showing row highlight
...
### 3. REQ-007 — Export / Save toolbar + Home library refresh in place
...
### 7. REQ-006 — Drag-and-drop reorder
**Gap:** ↑/↓ buttons still present (`moveSlide`); no drag handles or DnD library.

### 8. REQ-008 — Unit test coverage for REQ-001–007
**Gap:** Tests cover PR #12 baseline but not song-finish navigation, Showing, collapse, DnD, Export/Save rename, or Home in-place library update.

---

## Selected next item

**REQ-001: Fix song advance so that after the last lyric stage, Next / right-arrow / Advance advance to the next deck slide (never restart the song).**

**Why this item:** It is the known live-show bug called out in the epic and mock helper text (“After a song's last lyric, Next goes to the following slide”). It blocks correct operator workflow during presentation and is the smallest shippable increment with clear acceptance criteria. Implement shared step-forward logic in `DeckBuilder.tsx` (and add a failing-then-passing unit test in `DeckBuilder.unit.test.tsx`) before UI polish (REQ-002–007).
