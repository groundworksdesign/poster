...
1. [ ] `remix-vite-setup`: Scaffold Remix + Vite and port `/deck` + `/presentation` routes.
...
5. [ ] `broadcast-partial-updates`: Harden `BroadcastChannel` handling so navigation-only commands (lyricsNavigation) do not unintentionally clear slide/message.
